...
Item {
    ...
    function eClear() {
        for(eI = 0; eI < 16; eI++) {
            ...
            panel.eNums.itemAt(eI).color = "#cabeb2";
    ...
    Keys.onPressed: {
        switch(event.key) {
        case Qt.Key_Up:
            numProvider.move(ZHANG2048.Move_Up);
            root.eShow();
            break;
        case Qt.Key_Down:
            numProvider.move(ZHANG2048.Move_Down);
            root.eShow();
            break;
        case Qt.Key_Left:
            numProvider.move(ZHANG2048.Move_Left);
            root.eShow();
            break;
        case Qt.Key_Right:
            numProvider.move(ZHANG2048.Move_Right);
            root.eShow();
            break;
        default:
            break;
